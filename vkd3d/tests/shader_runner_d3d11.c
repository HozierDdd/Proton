...
static bool is_amd_windows_device(ID3D11Device *device)
{
    DXGI_ADAPTER_DESC desc;
...
static BOOL init_test_context(struct d3d11_shader_runner *runner)
{
    ...
    runner->caps.shader_caps[SHADER_CAP_STENCIL_EXPORT] = options2.PSSpecifiedStencilRefSupported;
    ...
    if (is_amd_windows_device(runner->device))
        ...
        const struct resource_params *params)
{
    ...
    resource->buffer = create_buffer(device, D3D11_BIND_SHADER_RESOURCE, params->desc.width * params->desc.texel_size,
            params->is_raw, params->stride, params->data);
    ...
    if (params->is_raw)
    {
        ...
        srv_desc.BufferEx.NumElements = params->desc.width;
    ...
    else
    {
        ...
        srv_desc.Buffer.NumElements = params->desc.width;
    }
        ...
        const struct resource_params *params)
{
    ...
    resource->buffer = create_buffer(device, D3D11_BIND_UNORDERED_ACCESS, params->desc.width * params->desc.texel_size,
            params->is_raw, params->stride, params->data);
    resource->resource = (ID3D11Resource *)resource->buffer;
    ...
    uav_desc.Buffer.NumElements = params->desc.width;
    if (params->is_raw)
...
static struct resource *d3d11_runner_create_resource(struct shader_runner *r, const struct resource_params *params)
{
    ...
    switch (params->desc.type)
    {
        ...
        case RESOURCE_TYPE_VERTEX_BUFFER:
            resource->buffer = create_buffer(device, D3D11_BIND_VERTEX_BUFFER,
                    params->desc.width * params->desc.texel_size, params->is_raw, params->stride, params->data);
            resource->resource = (ID3D11Resource *)resource->buffer;
            break;
...
static bool d3d11_runner_dispatch_mesh(struct shader_runner *r, unsigned int x, unsigned int y, unsigned int z)
{
    fatal_error("Mesh shaders are not supported.\n");
}

static void d3d11_runner_clear(struct shader_runner *r, struct resource *res,
        const struct vec4 *clear_value, uint32_t stencil)
{
    ...
    switch (resource->r.desc.type)
    {
        ...
        case RESOURCE_TYPE_DEPTH_STENCIL:
            ID3D11DeviceContext_ClearDepthStencilView(context, resource->dsv,
                    D3D11_CLEAR_DEPTH | D3D11_CLEAR_STENCIL, clear_value->x, stencil);
            break;
    ...
    for (i = 0; i < runner->r.resource_count; ++i)
    {
        ...
        switch (resource->r.desc.type)
        {
            ...
            case RESOURCE_TYPE_DEPTH_STENCIL:
                ...
                ds_desc.StencilEnable = runner->r.stencil_enable;
                ds_desc.StencilReadMask = D3D11_DEFAULT_STENCIL_READ_MASK;
                ds_desc.StencilWriteMask = D3D11_DEFAULT_STENCIL_WRITE_MASK;
                ds_desc.FrontFace.StencilFailOp = runner->r.stencil.StencilFailOp;
                ds_desc.FrontFace.StencilDepthFailOp = runner->r.stencil.StencilDepthFailOp;
                ds_desc.FrontFace.StencilPassOp = runner->r.stencil.StencilPassOp;
                ds_desc.FrontFace.StencilFunc = runner->r.stencil.StencilFunc;
                ...
                ID3D11DeviceContext_OMSetDepthStencilState(context, ds_state, runner->r.stencil_ref);
                if (resource->r.desc.width < fb_width)
                    fb_width = resource->r.desc.width;
                if (resource->r.desc.height < fb_height)
                    fb_height = resource->r.desc.height;
...
static const struct shader_runner_ops d3d11_runner_ops =
{
    ...
    .dispatch_mesh = d3d11_runner_dispatch_mesh,

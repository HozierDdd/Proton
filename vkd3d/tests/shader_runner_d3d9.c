...
static struct resource *d3d9_runner_create_resource(struct shader_runner *r, const struct resource_params *params)
{
    ...
    switch (params->desc.type)
    {
        ...
        case RESOURCE_TYPE_VERTEX_BUFFER:
            hr = IDirect3DDevice9_CreateVertexBuffer(device, params->desc.width * params->desc.texel_size,
...
static bool d3d9_runner_dispatch_mesh(struct shader_runner *r, unsigned int x, unsigned int y, unsigned int z)
{
...
static void d3d9_runner_clear(struct shader_runner *r, struct resource *res,
        ...
        unsigned int vertex_count, unsigned int instance_count, unsigned int first_vertex, unsigned int first_instance)
{
    ...
    u = &runner->r.uniform_buffers[VKD3D_SHADER_D3DBC_BOOL_CONSTANT_REGISTER];
    if (u->count)
    {
        hr = IDirect3DDevice9_SetVertexShaderConstantB(device, 0, (const BOOL *)u->elements, u->count);
        ok(hr == D3D_OK, "Failed to set bool constants, hr %#lx.\n", hr);
        hr = IDirect3DDevice9_SetPixelShaderConstantB(device, 0, (const BOOL *)u->elements, u->count);
        ok(hr == D3D_OK, "Failed to set bool constants, hr %#lx.\n", hr);
    }
...
static const struct shader_runner_ops d3d9_runner_ops =
{
    ...
    .dispatch_mesh = d3d9_runner_dispatch_mesh,

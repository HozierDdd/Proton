...
static bool check_gl_extensions(struct gl_runner *runner)
{
    ...
    if (check_gl_extension("GL_ARB_shader_stencil_export", count) && (runner->language == GLSL
            || check_spirv_extension("SPV_EXT_shader_stencil_export", spirv_count)))
...
static const struct format_info *get_format_info(enum DXGI_FORMAT format, bool is_shadow)
{
    ...
    static const struct format_info format_info[] =
    {
        ...
        {DXGI_FORMAT_R8G8B8A8_UNORM,     4, true,  false, GL_RGBA8,              GL_RGBA,            GL_UNSIGNED_BYTE},
        {DXGI_FORMAT_R8G8B8A8_UINT,      4, true,  false, GL_RGBA8UI,            GL_RGBA_INTEGER,    GL_UNSIGNED_BYTE},
        ...
        {DXGI_FORMAT_R8_UINT,            1, true,  false, GL_R8UI,               GL_RED_INTEGER,     GL_UNSIGNED_BYTE},
        {DXGI_FORMAT_D32_FLOAT_S8X24_UINT, 2, true, false, GL_DEPTH32F_STENCIL8, GL_DEPTH_STENCIL,   GL_FLOAT_32_UNSIGNED_INT_24_8_REV},
        {DXGI_FORMAT_X32_TYPELESS_G8X24_UINT, 2, true, false, GL_DEPTH32F_STENCIL8, GL_DEPTH_STENCIL,   GL_FLOAT_32_UNSIGNED_INT_24_8_REV},
    };
...
static bool gl_runner_init(struct gl_runner *runner, enum shading_language language)
{
    ...
    for (unsigned int i = 0; i < count; ++i)
    {
        ...
        if (strstr((const char *)glGetString(GL_RENDERER), "radeonsi"))
            runner->caps.tags[SHADER_RUNNER_TAG_RADEONSI].present = true;
...
static bool init_resource_texture(struct gl_resource *resource, const struct resource_params *params)
{
    ...
    if (resource->format->format == GL_DEPTH_STENCIL)
    {
        glTexParameteriv(target, GL_TEXTURE_SWIZZLE_RGBA, (const GLint[]){GL_ZERO, GL_RED, GL_ZERO, GL_ZERO});
...
static void init_resource_buffer(struct gl_resource *resource, const struct resource_params *params)
{
    ...
    glBufferData(target, params->desc.width * params->desc.texel_size, NULL, GL_STATIC_DRAW);
    if (params->data)
        glBufferSubData(target, 0, params->data_size, params->data);
...
static struct resource *gl_runner_create_resource(struct shader_runner *r, const struct resource_params *params)
{
    ...
    switch (params->desc.type)
    {
        ...
        case RESOURCE_TYPE_VERTEX_BUFFER:
            ...
            glBufferData(GL_ARRAY_BUFFER, params->desc.width * params->desc.texel_size, NULL, GL_STATIC_DRAW);
            if (params->data)
                glBufferSubData(GL_ARRAY_BUFFER, 0, params->data_size, params->data);
            break;
...
static bool gl_runner_dispatch_mesh(struct shader_runner *r, unsigned int x, unsigned int y, unsigned int z)
{
...
static GLenum get_stencil_op_gl(D3D12_STENCIL_OP op)
{
    switch (op)
    {
...
static void gl_runner_clear(struct shader_runner *r, struct resource *res,
        const struct vec4 *clear_value, uint32_t stencil)
{
    ...
    switch (resource->r.desc.type)
    {
        ...
        case RESOURCE_TYPE_DEPTH_STENCIL:
            ...
            if (resource->format->format == GL_DEPTH_STENCIL)
            {
                glFramebufferTexture(GL_FRAMEBUFFER, GL_STENCIL_ATTACHMENT, resource->id, 0);
                glStencilMask(~0u);
        ...
        switch (resource->r.desc.type)
        {
            ...
            case RESOURCE_TYPE_DEPTH_STENCIL:
                ...
                if (runner->r.stencil_enable)
                {
                    ...
                    glStencilFunc(get_compare_op_gl(runner->r.stencil.StencilFunc), runner->r.stencil_ref, 0xff);
                    glStencilOp(get_stencil_op_gl(runner->r.stencil.StencilFailOp),
                            get_stencil_op_gl(runner->r.stencil.StencilDepthFailOp),
                            get_stencil_op_gl(runner->r.stencil.StencilPassOp));
                    glStencilMask(0xff);
                }
                else
                {
...
static const struct shader_runner_ops gl_runner_ops =
{
    ...
    .dispatch_mesh = gl_runner_dispatch_mesh,
